        ...
        <div class="card mb-4">
            ...
            <div class="card-body p-0">
                <table class="table table-striped mb-0">
                    ...
                    <tbody>
                        <tr v-for="s in workshopStore.sessions" :key="s.id">
...
onMounted(async () => {
    ...
    workshopStore.sessions = workshopStore.currentWorkshop?.workshopSessions || []
})
...
async function saveSession() {
    // Formater la date pour Laravel : YYYY-MM-DD HH:mm
    const payload = {
        ...form,
    ...
    // Recharger les sessions pour récupérer les remaining_places et reservations_count
    await workshopStore.fetchWorkshopById(workshopId)
    workshopStore.sessions = workshopStore.currentWorkshop?.workshopSessions || []
    resetForm()
}
...
function editSession(session: any) {
    ...
    // Formater la date pour input type datetime-local
...
async function deleteSession(id: number) {
    if (confirm("Supprimer cette session ?")) {
        ...
        // Mettre à jour les sessions après suppression
